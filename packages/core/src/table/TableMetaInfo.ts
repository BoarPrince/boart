...
export class TableMetaInfo {
   ...
   *
   */
  static get<T extends AnyBaseRowType>(
    type: new (BaseTableRowMetaDefinition) => T
  ): MetaInfo {
  ...
  static getByInstance<T extends AnyBaseRowType>(instance: T): MetaInfo {
    ...
    if (!metaInfo.key) {
      throw Error(
        `key is not defined for table definition '${instance.constructor.name}'`
      );
    }
    if (!metaInfo.values || metaInfo.values.length === 0) {
      throw Error(
        `no value defined for table definition '${instance.constructor.name}'`
      );
    }
    return metaInfo;
  }
