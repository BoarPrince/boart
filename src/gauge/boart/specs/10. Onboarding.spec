...
## 1.1.1 Onboarding Using Rest API (manualy)

tags: ob-10.1.1, manual, env:development, env:staging, env:local
...
## 1.2. Send Onboarding Events and check id's (manualy, using queue)
...
## 1.2.1. Send Onboarding Events and check id's (manualy, using rest)

tags: ob-10.2.1, manual, env:staging, env:development
...
## 1.5. Id's must match after onboarding event when debtor already exists (tax - match, manualy)
...
## 1.6. If portal is sending the event (fleet event bus) more than one time, the carrier must always be the same (manualy)
