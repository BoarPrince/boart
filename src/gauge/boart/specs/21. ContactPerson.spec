...
* Rest call
   ...
   | --expected#addresses   | [${store:contactPersonId}]                     |
   | expected#phoneNumbers  | []                                             |
   | expected#languageCode  | DK                                             |

## 1.2-1. Add a Contact Person (add Address afterwards)

tags: md-21.2, md-21.2-1
...
* Rest call
   ...
   | expected#lastName      | last                                          |
   | expected#firstName     | first                                         |
   | expected#addresses[0]  | ${store:response-cp.id}                       |
   | expected#phoneNumbers  | []                                            |
   | expected#languageCode  | DK                                            |
...
* Rest call
   ...
   | action                 | value                                                       |
   |------------------------|-------------------------------------------------------------|
   | method:get             | /api/v2/address/${store:response-cp.id}                     |
   | description            | Read the address implicit generated with the contact person |
   | expected:header#status | 200                                                         |
   | store                  | response-cp                                                 |
...
* Rest call
   ...
   | action                 | value                                         |
   |------------------------|-----------------------------------------------|
   | method:put             | /api/v2/address/${store:response-cp.id}       |
   | description            | Update the address with the correct companyId |
   | payload                | ${store:response-cp}                          |
   | payload#companyId      | ${store:response-co.id}                       |
   | payload#zipCode        | zipCode-1                                     |
   | payload#city           | city-1                                        |
   | payload#street         | street-1                                      |
   | expected:header#status | 200                                           |
   | store                  | response-cp                                   |
...
* Rest call
   ...
   |----------------------|-----------------------------------------|
   | method:get           | /api/v2/address/${store:response-cp.id} |
   | description          | Read the updated address again          |
   | expected#type        | type1                                   |
   | expected#street      | street-1                                |
   | expected#city        | city-1                                  |
   | expected#zipCode     | zipCode-1                               |
   | expected#countryCode | DK                                      |
   | expected#companyId   | ${store:response-co.id}                 |

## 1.2-2. Add a Contact Person (with additional Address and without PhoneNumbers)

tags: md-21.2, md-21.2-2
...
* Rest call
   ...
   | expected#lastName           | last                                          |
   | expected#firstName          | first                                         |
   | expected#addresses[0]       | ${store:response-ad.id}                       |
   | expected#phoneNumbers[0].id | ${store:response-ca.phoneNumbers[0].id}       |
   | expected#languageCode       | DK                                            |

## 1.2-3. Add a Contact Person (with Address and with PhoneNumber)

tags: md-21.2, md-21.2-3

* Test description

   | action      | value                                                            |
   |-------------|------------------------------------------------------------------|
   | description | Add a new Contact Person with an address abd with a phone number |
   | priority    | high                                                             |
...
* Rest call
   ...
   |------------------------------------------|---------------------------------------------------------------------|
   | method:post                              | /api/v2/contactPerson                                               |
   | description                              | Creates a ContactPerson with a Phonenumber and associated Addresses |
   | payload#type                             | type1                                                               |
   | payload#email                            | email                                                               |
   | payload#lastName                         | last                                                                |
   | payload#firstName                        | first                                                               |
   | payload#languageCode                     | DK                                                                  |
   | payload#addresses[0]                     | ${store:response-ad.id}                                             |
   | payload#phoneNumbers[0].number           | ${generate:random:10}                                               |
   | payload#phoneNumbers[0].countryPhoneCode | ${generate:random:3}                                                |
   | expected:header#status                   | 200                                                                 |
   | store                                    | response-cp                                                         |
...
* Rest call
   ...
   | description                               | Read the previosly created contact person            |
   | expected:header#status                    | 200                                                  |
   | expected#phoneNumbers[0].number           | ${store:payload-cp.phoneNumbers[0].number}           |
   | expected#phoneNumbers[0].countryPhoneCode | ${store:payload-cp.phoneNumbers[0].countryPhoneCode} |

## 1.2-4. Try add a Contact Person (without Address and with PhoneNumber)

tags: md-21.2, md-21.2-4

* Test description

   | action      | value                                                               |
   |-------------|---------------------------------------------------------------------|
   | description | Add a new Contact Person without an address abd with a phone number |
   | priority    | high                                                                |
...
* Rest call
   ...
   | expected#lastName                           | last                                           |
   | expected#firstName                          | first                                          |
   | expected#addresses[0]                       | ${store:contactPersonId}                       |
...
* Rest call
   ...
   | payload#lastName       | ${generate:fake:name:lastName}-${generate:random:5}  |
   | payload#firstName      | ${generate:fake:name:firstName}-${generate:random:5} |
...
* Rest call
   ...
   | method:post            | /api/v2/contactPerson                                   |
   | description            | Creates a ContactPerson with random values              |
   |                        | Take the same v1 instance)as the created address before |
   | payload#type           | ${generate:char:20}                                     |
   | payload#email          | ${generate:fake:internet:email}                         |
...
* Rest call
   ...
   |------------------------|-----------------------------------------------|
   | method:get             | /api/v2/contactPerson/${store:response-cp.id} |
   | description            | Read previously created contact person        |
   | expected:header#status | 200                                           |
   | expected#id            | ${store:addressId}                            |
...
## 1.7. Id must not defined for Update
...
* Test description
   ...
   | action      | value                                                         |
   |-------------|---------------------------------------------------------------|
   | description | Checks the correct error message when id is defind in payload |
   | priority    | high                                                          |
...
* Rest call
   ...
   | description            | Update ContactPerson wit id in payload        |
   | payload                | ${store:response-cp}                          |
   | expected:header#status | 406                                           |
...
* RabbitMQ publish
   ...
   | action            | value                                         |
   |-------------------|-----------------------------------------------|
   | description       | CREATE: Publish contract person event manualy |
...
* RabbitMQ publish
   ...
   | action            | value                                                 |
   |-------------------|-------------------------------------------------------|
   | description       | UPDATE: Publish contract person updated event manualy |
...
* RabbitMQ publish
   ...
   | action            | value                                                 |
   |-------------------|-------------------------------------------------------|
   | description       | DELETE: Publish contract person updated event manualy |
