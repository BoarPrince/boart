...
import { ASTSelectorType, DefaultOperator, EnvLoader, GeneratorHandler, OperatorType, ScopedType, ScopeType, SelectorType, Store, TextLanguageHandler, ValueReplacer } from '@boart/core';
...
jest.mock('@boart/core', () => {
     ...
     *
     */
    const storeFactory = (baseStore: Map<string, string>, name: string) => ({
    ...
    return {
        __esModule: true,
        ...originalModule,
        StoreWrapper: jest.fn().mockImplementation(() => storeFactory(scenarioMap, '-x-')),
        Store: {
            instance: {
                globalStore: storeFactory(suiteMap, 'global'),
                localStore: storeFactory(specMap, 'local'),
                testStore: storeFactory(scenarioMap, 'test'),
                stepStore: storeFactory(stepMap, 'step')
            }
        },
        EnvLoader: {
            instance: {
...
describe('generate', () => {
    ...
    it('default', () => {
        ...
        expect(store.store.get).toBeCalledWith('#generate#:#test#:#xxxx#');
    });
    ...
    it('with arguments', () => {
        ...
        expect(store.store.get).toBeCalledWith('#generate#:#test#:#xxxx:arg1:arg2#');
    });
});
