...
describe('checkbox-proxy', () => {
     ...
     *
     */
    test('must be a checkbox proxy', async () => {
        await pageAdapter.driver.html(`<input type="checkbox" id="test-id" value="test-value">`);
        const element = await pageAdapter.element.byId('test-id');

        const sut = await pageAdapter.element.getProxy(element);
        expect(sut.name).toBe('checkbox');
    });
    ...
    test('must not be visible', async () => {
        await pageAdapter.driver.html(`<input hidden type="checkbox" id="test-id" value="test-value">`);
        const element = await pageAdapter.element.byId('test-id');
    ...
    test('must not be enabled', async () => {
        await pageAdapter.driver.html(`<input disabled type="checkbox" id="test-id" value="test-value">`);
        const element = await pageAdapter.element.byId('test-id');
    ...
    test('text must not be available', async () => {
        await pageAdapter.driver.html(`<input checked type="checkbox" id="test-id" value="test-value">`);
        const element = await pageAdapter.element.byId('test-id');
    ...
    test('text must be available', async () => {
        ...
        <input checked type="checkbox" id="test-id" value="test-value">
    ...
    test('classes must be visible', async () => {
        await pageAdapter.driver.html(`<input checked type="checkbox" id="test-id" value="test-value" class="test1 test2">`);
        const element = await pageAdapter.element.byId('test-id');
    ...
    test('element info must be accessible', async () => {
        await pageAdapter.driver.html(`
          <input checked type="checkbox" id="test-id" value="test-value" class="clazz1 clazz2">
          <label for="test-id">Test-Value</label>`);
        const element = await pageAdapter.element.byId('test-id');
    ...
    test('click must work', async () => {
        await pageAdapter.driver.html(`<input type="checkbox" id="test-id" value="test-value" class="test1 test2">`);
     ...
     *
     */
    test('click must trigger click handler', async () => {
        await pageAdapter.driver.html(`
        <input type="checkbox" id="test-id" value="test-value" onclick="clickHandler()"></input>
        <div id="test-id-div"></input>

        <script>
          function clickHandler() {
            document.getElementById("test-id-div").innerHTML = "has clicked!";
          }
        </script>
      `);
        ...
        const elementDiv = await pageAdapter.element.byId('test-id-div');
        const elementDivProxy = await pageAdapter.element.getProxy(elementDiv);
        await expect(elementDivProxy.getValue(elementDiv)).resolves.toBe('has clicked!');
    });
});
